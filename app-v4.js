...
function buildSharedResultUrl() {
  ...
  url.searchParams.set('food', foodStr);
  return url.toString();
}
...
async function shareDateResult() {
  ...
  const shareUrl = buildSharedResultUrl();
  window.history.replaceState(null, '', shareUrl);

  if (isWechatBrowser()) {
    const guide = document.getElementById('wechat-share-guide');
    if (guide) guide.classList.add('active');
    showShareStatus('请点微信右上角「…」转发给朋友');
    return;
  }

  if (navigator.share) {
    try {
      await navigator.share({ title: '我们的约会安排', text, url: shareUrl });
...
function loadSharedResult() {
  ...
  const foodStr = params.get('food');
  if (!dateStr || !/^\d{4}\/\d{2}\/\d{2}$/.test(dateStr) ||
      !timeStr || !/^([01]\d|2[0-3]):[0-5]\d$/.test(timeStr) || !foodStr) return false;

  AppState.selectedDate = dateStr;
  AppState.selectedTime = timeStr;
  AppState.selectedFood = foodStr.slice(0, 30);
  localStorage.setItem('dating_selectedDate', AppState.selectedDate);
  localStorage.setItem('dating_selectedTime', AppState.selectedTime);
  localStorage.setItem('dating_selectedFood', AppState.selectedFood);
  return true;
}
...
// ── 初始化 ──
function initApp() {
  const hasSharedResult = loadSharedResult();
  // 默认显示第一页
  const pages = document.querySelectorAll('.app-page');
  pages.forEach(p => {
    p.style.display = 'none';
  ...
  if (hasSharedResult) navigateTo('confirm');
